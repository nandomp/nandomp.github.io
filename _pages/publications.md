...
{% comment %}
  ...
  Every entry renders with the same structure:
    Authors. “Title”. Venue, details, Year.  [type] [DOI] [PDF] [Code] …
{% endcomment %}
...
<p class="pub-item__links"><span class="badge badge--type">{{ pub.type | capitalize }}</span>{% if pub.award %}{% if pub.award_url %}<a class="badge badge--award" href="{{ pub.award_url }}">{{ pub.award }}</a>{% else %}<span class="badge badge--award">{{ pub.award }}</span>{% endif %}{% endif %}{% for link in pub.links %}<a class="badge badge--action" href="{{ link.url }}">{{ link.label }}</a>{% endfor %}</p>
</article>
{%- endfor %}
</section>
{%- endfor %}
